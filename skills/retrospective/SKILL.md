...
Q3: この改善は、このリポジトリの Hikyaku ワークフロー全体（複数フェーズ）に適用されるか？
  → Yes → 対象: workflow:instructions.md
  → No ↓
...
| `workflow:instructions.md` | ワークフロー固有のインストラクション修正 | `instructions.md` に追記する内容の文面案を書く |
...
**「改善提案」と「昇格候補」を必ず分けること。** 前者は Hikyaku スキル自体への提案で
retrospective.md に留まり、後者はこのリポジトリ固有の学びとして close-cycle が
`learnings` へ昇格させる。この節が無いと、close-cycle の素材収集で拾うものが無くなる。

再現条件が具体的に書けないものは昇格候補にしない。一般的なプログラミング知識や
「気をつける」だけの曖昧な注意も対象外。
...
- **昇格候補は再現条件で判定する。** 具体的に書けないものは書かない。close-cycle が
