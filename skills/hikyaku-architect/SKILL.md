...
description: "Hikyaku 設計フェーズ: 企画フェーズの成果物を入力とし、技術設計・タスク分割・タスク定義を実行する。企画(planning/)が完了し、技術設計とタスク分割を行いたいときに使う。"
user-invocable: true
...
BP見積もりガイド（[bp-guide.md](references/bp-guide.md)）に基づいてタスクを定義する。
...
`$ARGUMENTS/tasklist.md` にタスク一覧を記録する（フォーマットは [templates.md](references/templates.md) を参照）

### Step 6: タスク定義

tasklist.md の各タスクについて、`$ARGUMENTS/build-{NN}/issue.md` を作成する（フォーマットは [templates.md](references/templates.md) を参照）。
