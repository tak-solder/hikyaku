...
**判定基準は「依存ビルドの成果が、いま居る作業ツリーに在るか」。** デフォルトブランチ
経由でマージされていても、先行ビルドのブランチから積んで（スタックして）いても着手できる。
`next` は作業ツリーの tasklist.md の `PR` 列でこれを判定する。待機中と表示された
ビルドには着手しない。
...
伝える（ブランチ名から導出しているため）。**着手可能・待機の判定と中断検出には影響しない。**
...
着手可能の判定は作業ツリーが基準なので、ブランチを切り替えると答えが変わる。
...
- [ ] レビューの差分基準にするブランチを取得し、`{BASE_BRANCH}` として渡す

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/hikyaku.mts" pr base build-{NN} {cycle}
```

**デフォルトブランチを決め打ちしない。** スタックしている場合はスタック元が返る。
デフォルトブランチを基準にすると、先行ビルドの差分までレビュー対象に混入する。
...
返った値を PR の base にする。**デフォルトブランチを決め打ちしない。**
...
どちらでも `next` は着手可能と判定する（判定は作業ツリーに成果が在るかで行うため）。
