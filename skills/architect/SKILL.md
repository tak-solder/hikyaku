...
- [ ] **調査範囲の判定**
  - user-stories.md が単一機能・小規模スコープ → **単一エージェント** で調査
  - user-stories.md が複数の独立した機能領域を含む、または既存コードベースが大規模 → **複数エージェントを観点別に並列起動**（2〜3つ）

- [ ] **code-explorer エージェントを起動する**（単一または並列）
  - 並列の場合の観点例:
    - 「user-stories の `US-X` に類似する既存機能のトレース」
    - 「user-stories 全体に関連するアーキテクチャ層・抽象化の把握」
    - 「拡張ポイント（フック・インターフェース・抽象クラス）の特定」
  - 各エージェントには以下を必ず返させる:
    - 担当観点の調査結果（`codebase-survey.md` のセクション断片として転記しやすい形式）
    - **Key Files**: その観点で本セッションが直接読むべき5〜10ファイルのパスと役割
  - 各観点の出力フォーマットは `agents/code-explorer.md` を参照
  - 加えて、いずれかのエージェントには **リポジトリ構成の概観**（ディレクトリツリー depth 2-3、設定ファイルから検出した技術スタック）の取得も担当させる
...
- [ ] **`codebase-survey.md` の作成**
  - 単一エージェントの場合: エージェントの出力をそのまま `$ARGUMENTS[0]/architecture/codebase-survey.md` に書き出す（必要に応じてメインセッションで整形）
  - 並列の場合: 各エージェントの結果と Key Files から得た知見を統合し、メインセッションで `codebase-survey.md` を作成する
