...
argument-hint: "{path/to/project} [buildID]"
metadata:
...
`$ARGUMENTS[0]` のワークフローパスから企画・設計ドキュメントを読み込み、実装フェーズを実行する。`$ARGUMENTS[1]` で buildID を指定できる（省略時は次のタスクを自動選択）。
...
- `$ARGUMENTS[1]` が省略または `next` の場合: PR列が空かつ、依存タスク（dependencies列）のPR列がすべて埋まっているタスクの中から、最小のbuildIDを選択する
