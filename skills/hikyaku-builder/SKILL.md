...
flowchart TD
    ...
    S4b --> S4
    S4a -- No --> S5["Step 5: Agent が test-spec.md を生成"]
    S5 --> S6["Step 6: plan.md + test-spec.md のユーザー承認"]

    S6 --> S6a{"承認?"}
    S6a -- No --> S6b["フィードバックを反映"]
    S6b --> S6c{"plan.md に変更あり?"}
    S6c -- Yes --> S5
    S6c -- No --> S6
    S6a -- Yes --> S7["Step 7: コード生成"]
    S7 --> S8["Step 8: ローカル検証"]
    S8 --> S8a{"全パス?"}
    S8a -- No --> S8b["エラー修正（retry-policy に従う）"]
    S8b --> S8
...
### Step 6: plan.md + test-spec.md のユーザー承認

plan.md と test-spec.md の内容をユーザーに提示し、承認を得る。
...
**フィードバック時の対応:**
- plan.md に変更が生じた場合: plan.md を修正した上で、Step 5 に戻り Agent で test-spec.md を再生成する
...
| Step 7（コード生成）中 | 想定外の複雑さや未定義の依存が判明 | ビルドの追加・更新 |
| Step 9（申し送り）時 | 意図的に先送りした作業を新ビルドとして記録 | ビルドの追加 |

**build-manager 呼び出し後の対応:**
- 現在のビルドのスコープが変更された場合: plan.md を修正し、必要に応じて test-spec.md を再生成する（Step 5 に戻る）
