...
| `doc-reviewer` | planner / architect / build-manager / builder | 中間成果物の整合性・網羅性・曖昧さを証拠ベースで報告する |
...
`doc-reviewer` は渡された context（`user-stories` / `architecture` / `tasklist` / `plan` / `test-spec`）に応じて観点を切り替えます。architecture と plan ではセキュリティ設計の考慮漏れも見ます。tasklist は BP見積もりの妥当性と分割の網羅性が対象で、tasklist.md・issue.md がまだファイルに書き込まれていない段階でレビューするため、build-manager が内容をプロンプトへ直接渡します。test-spec は plan.md・issue.md に対するテストシナリオの網羅性が対象で、express / standard では plan.md 作成直後の `context: plan` レビューと分けて、test-spec.md 作成後に呼ばれます（thorough では test-spec.md 作成後の `context: plan` レビューにまとめられ、test-spec 単独のレビューは行いません）。
