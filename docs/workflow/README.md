...
PLAN / ARCHITECT / CLOSE の PR はドキュメントのみで、速やかにマージすることを想定しています。とくに ARCHITECT の PR を放置すると、`tasklist.md` が main に無いあいだサイクルの進捗が外から見えなくなります。

BUILD の PR にはコードと `tasklist.md` の `PR` 列の更新が同梱されます。この同梱があるから、`PR` 列が埋まっていること自体が「その実装がこのツリーに在る」ことを意味し、main 上で埋まっていれば「マージ済み」を意味します。
...
規則から外れたブランチで作業すると、`next` の「着手中」表示と、`hikyaku pr base` によるスタック元の導出が効かなくなります。どちらもブランチ名から導出しているためです。着手可能・待機の判定（作業ツリーの `PR` 列）と中断検出（成果物の有無）には影響しません。
