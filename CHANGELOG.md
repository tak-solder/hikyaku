...
- **ブランチ規則検証メッセージ**: ユーザーが Hikyaku の命名規則に沿わないブランチにいる場合のメッセージを簡潔にした
  - 推測による進行を避け、明確な指示を提示するようにした

### Migration

- 利用者側での対応は不要。メッセージの表示内容が変わるのみ
...
- **`hikyaku pr base <phase> [<cycle>]`**: PR のマージ先ブランチを返す。先行フェーズのブランチから積んでいる（スタックしている）場合はそのブランチを、そうでなければデフォルトブランチを返す。スタック元は状態として保存せず、「同じサイクルの Hikyaku ブランチのうち、HEAD の履歴に含まれていて、まだ base に取り込まれていない、最も近いもの」として導出する。マージ済みのブランチも HEAD の祖先になるため、取り込み済みを除かないと「デフォルトブランチから切っただけ」をスタックと誤判定する
  - 取り込み済みの判定は、祖先関係とデフォルトブランチの `PR` 列を併用する。**squash merge / rebase merge ではマージ済みでもブランチの先端が base の祖先にならない**ため、祖先関係だけでは既に削除されたブランチを PR の base に指名してしまう
  - スタック元が origin に存在しない場合は警告する
  - マージ状況を確認できない（デフォルトブランチの tasklist も base の ref も読めない）場合はスタック元を推測せず、デフォルトブランチを返す
  - `--ref` でローカルに解決できる ref を返す。PR の base に渡すのはブランチ名だが、`git merge-base` に渡すには解決できる ref が要る（リモート追跡参照しか無いブランチを名前のまま渡すと解決に失敗し、コミット済み差分が空のままレビューされる）
- **`branch verify` の `stackedOn` / `prBase`**: スタック元と、PR のマージ先を出力に加えた。builder はブランチ決定時にこれを見て、スタックになることをユーザーへ伝える

### Changed

- **着手可能・待機の判定を `HEAD` 基準にした**: 問うているのは「依存ビルドの成果が、いま居るブランチの履歴に在るか」であって、デフォルトブランチにマージされたかではない。マージでも、先行ビルドのブランチから積む（スタックする）ことでも着手できる。`PR` 列の更新は実装と同じコミットに同梱されるため、`HEAD` で `PR` 列が非空であることがそのまま実装が履歴に在ることを意味する。判定がリモート追跡参照とネットワークから完全に切り離され、古い `origin/{base}` で判定が変わることがなくなった
  - 作業ツリーのファイルではなく `HEAD` を読むため、**未コミットの編集は数えない**。`tasklist done` からコミットまでのあいだ、そのビルドは未完了に見える
  - `tasklist.md` を3つの断面で読み、buildID で突き合わせる。一覧と依存グラフは作業ツリー（まだコミットしていない追加分も候補に出すため）、`PR` 列（着手判定・中断点）は `HEAD`、`PR` 列（マージ済みラベル・`completed` 判定）は `origin/{base}`
- **`deriveState` を2つの問いに分けた**: `completed` かどうかはリポジトリ全体の問いなのでデフォルトブランチで、中断点の特定はツリーローカルの問いなので `HEAD` で判定する。同じ入力で両方を答えていたため、スタック中にマージ待ちの先行ビルドが「未完了」に混ざり、中断点が古いビルドへ戻っていた。デフォルトブランチを読めない場合は `completed` と判定しない（close-cycle を誤って勧めるほうが害が大きい）
- **`cycle list` / `cycle status` を `next` と同じ断面に揃えた**: 作業ツリーの `PR` 列でフェーズを判定していたため、最後のビルドで `tasklist done` した直後に `completed` と表示して close-cycle を勧めていた。ビルド列は「マージ済み / 全体」になり、手元で完了しマージ待ちのビルドは `(+n)` と `マージ待ち:` 行で示す
- **判定の出所を出力するようにした**: どの ref を、いつ時点の内容として読んだかを `next` と `cycle status` が表示する
- **`origin/{base}` が古いときだけ追跡参照を1本更新するようにした**: `git ls-remote` の結果（着手中の検出で既に取得済み）とローカルの `origin/{base}` を比べ、食い違うときだけ `git fetch origin +refs/heads/{base}:refs/remotes/origin/{base}` を実行する。作業ツリーにもローカルの `{base}` にも触らない。**着手判定はこの ref を見ないので、fetch の成否で着手できるかは変わらない**。`--no-fetch` で無効にできる
- **ref の読み取りで「ファイルが無い」と「ref が読めない」を区別するようにした**: 前者は「その断面には1件も無い」と確定できるため作業ツリーへ縮退しない。両方を同じ失敗として扱っていたため、ARCHITECT の PR が未マージなだけの状態で作業ツリーへ縮退し、ビルドブランチ上の自分の `PR` 列を「マージ済み」として拾っていた。判定は終了コードで行う（`fatal:` メッセージは locale 依存のため）
- **待機中のビルドについてもブランチの存在を表示するようにした**: 従来は着手可能なビルドしか照合していなかったため、他セッションが積んで作業中でも「待機中」としか見えなかった
- **`code-reviewer` / `security-reviewer` の差分基準を `origin/main` の決め打ちから `{BASE_BRANCH}` に変えた**: base はサイクルごとに変えられ、スタック時は先行ビルドのブランチになる。builder が `pr base` で導出して渡す
