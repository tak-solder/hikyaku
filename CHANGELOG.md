...
- **`next` の着手可能・待機の判定を作業ツリー基準にした**: 問うているのは「依存ビルドの成果が、いま居る作業ツリーに在るか」であって、デフォルトブランチにマージされたかではない。マージでも、先行ビルドのブランチから積む（スタックする）ことでも着手できる。判定がリモート追跡参照とネットワークから完全に切り離され、古い `origin/{base}` で判定が変わることがなくなった。デフォルトブランチの `tasklist.md` は「マージ済み」ラベルとフェーズ判定にだけ使う
- **`next` が判定の出所を出力するようにした**: 判定に使った作業ツリーのパスと、マージ状況を読んだ ref・SHA・コミット日時を添える。`origin/{base}` がリモートの先端と食い違う場合は警告する（`git ls-remote` の結果を再利用するため、通信は増えない）
- **デフォルトブランチの読み取りで「ファイルが無い」と「ref が読めない」を区別するようにした**: 前者は「マージ済み0件」と確定できるため作業ツリーへ縮退しない。両方を同じ失敗として扱っていたため、ARCHITECT の PR が未マージなだけの状態で作業ツリーへ縮退し、ビルドブランチ上の自分の `PR` 列を「マージ済み」として拾っていた。判定は終了コードで行う（`fatal:` メッセージは locale 依存のため）
