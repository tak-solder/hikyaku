...
**SP見積もり手順（タスクごとに実施）:**
1. 主要指標（新規ファイル数、実装行数、API操作数、画面数、DBテーブル数）を見積もり、最大値をベースSPとする
2. 加算要素を **必ず** チェックし、該当するものを加算する:
   - [ ] 基盤セットアップを含むか（+1）
   - [ ] 外部API連携を含むか（+1）
   - [ ] 既存コードの大規模リファクタを含むか（+1）
   - [ ] 複数パッケージにまたがるか（+ワークスペース数）
3. 合計SPが5を超える場合は分割を検討する

**SP 8 の許容基準:** 分割すると以下のいずれかに該当する場合のみ SP 8 を許容する:
- 分割したタスク間で同一ファイルの同一箇所を編集する必要があり、マージコンフリクトが避けられない
- 分割の一方が SP 1 未満になり、タスクとして成立しない
