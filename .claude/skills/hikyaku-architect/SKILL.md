...
description: "Hikyaku 設計フェーズ: 企画フェーズの成果物を入力とし、技術設計・タスク分割・タスク定義を実行する。"
user-invocable: true
...
/hikyaku-planner   → planning/ を生成（完了済み）
      ↓
/hikyaku-architect → architecture/ + tasklist.md + issue.md を生成  ← あなたはここ
      ↓ ユーザー承認
...
```
{path/to/project}/
├── tasklist.md               # タスク一覧（ARCHITECT で作成、BUILD で PR列を更新）
...
- タスクを分割し、tasklist.md と各タスクの issue.md を作成する
...
flowchart TD
    ...
    S4 --> S5["Step 5: タスク分割 → tasklist.md"]
    S5 --> S6["Step 6: タスク定義 → issue.md"]
    S6 --> S7["Step 7: ユーザー承認"]
    S7 --> S7a{"承認?"}
...
**既存コードがない（新規プロジェクト）場合はこのステップをスキップする。**
...
2. Agent に以下の調査を依頼し、結果を `$ARGUMENTS/architecture/codebase-survey.md` に書き出させる:
   - **プロジェクト構成の概観**: ディレクトリツリー（depth 2-3）、設定ファイル（package.json, CLAUDE.md 等）から技術スタックと全体構成を把握
...
| ドキュメント | 内容 | 作成基準 |
|------------|------|---------|
| tech-stack.md | 技術選択・依存ライブラリの決定 | 新規プロジェクト or 技術選定が必要な場合 |
| db-schema.md | DBスキーマ（テーブル定義・リレーション） | DBを使う場合 |
| interfaces.md | メソッドシグネチャ一覧 | API or 複数コンポーネント間の連携がある場合 |
| conventions.md | 共通設計方針・命名規約・ディレクトリ構成方針 | 新規プロジェクト or 新しい規約が必要な場合 |
...
**ディレクトリ構成について:**
- 既存プロジェクトの構成が明確な場合や、CLAUDE.md等で規約が定義済みの場合は検討不要
- 新規プロジェクトなど構成の判断が必要な場合は、conventions.md にディレクトリ構成方針を含める

各ドキュメントのフォーマットは [templates.md](templates.md) を参照。

### Step 5: タスク分割

- SP見積もりガイド（[sp-guide.md](sp-guide.md)）に基づいてタスクを定義する
- 各タスクがSP 5以下になるよう分割する（分割による損失が大きい場合はSP 8も許容）
- `$ARGUMENTS/tasklist.md` にタスク一覧を記録する（フォーマットは [templates.md](templates.md) を参照）

### Step 6: タスク定義

tasklist.md の各タスクについて、`$ARGUMENTS/build-{NN}/issue.md` を作成する（フォーマットは [templates.md](templates.md) を参照）。
...
**承認の原則**: 設計フェーズの決定は全タスクに波及するため、承認は **必須**。
