...
description: "Hikyaku 企画フェーズ: 既存の企画ドキュメントを読み込み、設計フェーズに渡せる形にユーザーストーリーとして構造化する。"
user-invocable: true
...
`$ARGUMENTS` に指定されたパスにプロジェクトドキュメントをセットアップし、企画フェーズを実行する。
...
/hikyaku-planner   → planning/ を生成
      ↓ ユーザー承認
/hikyaku-architect → architecture/ + tasklist.md + issue.md を生成
      ↓ ユーザー承認
