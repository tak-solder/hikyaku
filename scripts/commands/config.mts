...
register({
  ...
  details: [
    ...
    "個別キー（architecture_gate, plan_review, test_spec_review など）で上書きできます。",
  ...
  run: ({ args, operands }) => {
    ...
    emit({ ...config, cycle: opened?.context.name, cycleSource: opened?.context.source }, () => {
      ...
      lines.push(
        ...
        `  test_spec_review     ${onOff(config.reviews.testSpec)}`,
