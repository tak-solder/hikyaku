...
- 参照（ファイルパスで渡される）: `cycles/{cycle}/design/design-delta.md`（存在する場合）,
  BP の基準表（`{HIKYAKU_ROOT}/bp-guide/README.md`。無ければ委任元が `hikyaku bp guide --markdown` の
  出力をプロンプトに含める）
...
- 参照: `cycles/{cycle}/build-{NN}/issue.md`, `cycles/{cycle}/design/design-delta.md`,
  ...
  BP の基準表（`{HIKYAKU_ROOT}/bp-guide/README.md`。無ければ委任元が `hikyaku bp guide --markdown` の
  出力をプロンプトに含める）と `bp_max`（委任元プロンプトで渡される）

**BP の表への当てはめは `hikyaku bp estimate` が行っている。** レビューで見るのは**入力値**で、
表の読み違えではない。内訳表の「値」列（新規ファイル数、実装行数、影響ファイル数、加算要素の
有無）が、対象ドキュメントの記述から読み取れる規模と整合しているかを確かめる。
...
- **BP入力値の乖離**: issue.md のスコープ記述から推測される規模（新規ファイル数・実装行数・影響範囲・加算要素の該当）と、「BP見積もり」内訳表の入力値が明らかに乖離している。内訳表が無い、または「未指定」の指標が明らかに該当する場合も含む
  - 根拠: issue.md のスコープ記述と、内訳表の該当する行を示せる
...
- **BP入力値の乖離**: plan.md のクラス設計・実装ステップ・依存パッケージから読み取れる規模と、「BP見積もり」の内訳表の入力値（新規ファイル数・実装行数・影響ファイル数・加算要素の有無）が明らかに乖離している。列挙された「作成するファイル」とクラス設計が合っていない場合も含む
  - 根拠: plan.md の該当箇所（クラス設計・実装ステップ）と、内訳表の該当する行を示せる
  - **内訳表や「作成するファイル」の列挙が無い場合も、この根拠ラベルで報告する。** 根拠が無ければ乖離を検証できず、レビュー対象として成立しない
  - **加算要素の見落としを優先して見る。** 過小見積もりの多くはベースBPではなく、影響ファイル数・基盤セットアップ・外部API連携・大規模リファクタの取りこぼしから来る。内訳表で「未指定」の行が、plan.md の記述から見て該当するなら指摘する
